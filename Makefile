...
APP_NAME := tg-bot-accept-applications
APP_ROOT := src
...
build_base_image:
	eval `ssh-agent -s`; ssh-add /mnt/c/Users/mih2303/.ssh/id_rsa; DOCKER_BUILDKIT=1 docker build -f compose/base.Dockerfile --platform linux/amd64 --ssh default= --target base --add-host=gitea.rzd.energy:192.168.53.106 -t github.com/tg-bot-accept-applications:base .; ssh-agent -k
...
run_backend:  ## Запустить только backend API
	export IMAGE_NAME=$(call get_image_name) && \
	docker compose -f compose/docker-compose.$(ENV).yaml up -d tg-bot-accept-applications
...
logs_backend:
	docker logs -f tg-bot-accept-applications
...
shell:  ## Зайти в контейнер backend-а
	docker exec -it tg-bot-accept-applications bash
